...
function weightDeltaColor(delta: number): string {
...
function MetricCard({ label, value, unit, norm, color, accentSoft }: {
  label: string; value: number; unit: string; norm: number | null; color: string; accentSoft: string;
}) {
...
function CompactDaySummary({ cal, p, f, c }: { cal: number; p: number; f: number; c: number }) {
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, padding: '10px 16px', fontSize: 12, borderBottom: '1px solid var(--border)' }}>
      ...
      <span style={{ color: 'var(--text-2)' }}>Б <span style={{ fontWeight: 600, color: '#7EB8F0' }}>{Math.round(p)}</span>г</span>
      <span style={{ color: 'var(--text-2)' }}>Ж <span style={{ fontWeight: 600, color: '#F0A07A' }}>{Math.round(f)}</span>г</span>
      <span style={{ color: 'var(--text-2)' }}>У <span style={{ fontWeight: 600, color: '#90C860' }}>{Math.round(c)}</span>г</span>
    </div>
  );
}
      ...
      {groups.map(g => (
        <div key={g.type} style={{ background: 'var(--surface)', borderRadius: 'var(--r-lg)', border: '1px solid var(--border)', overflow: 'hidden' }}>
          <div style={{ padding: '8px 14px', borderBottom: '1px solid var(--border)' }}>
            <span style={{ fontSize: 12, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.7, color: 'var(--accent)' }}>
              {MEAL_TYPE_LABELS[g.type] ?? g.type}
            </span>
          </div>
          {g.entries.map((m: MealEntry) => {
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {m.caloriesKcal != null && (
                      <div style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 3 }}>
                        {Math.round(m.caloriesKcal)} ккал
                        {m.proteinG != null && ` · Б${m.proteinG.toFixed(0)} Ж${m.fatG?.toFixed(0)} У${m.carbsG?.toFixed(0)}`}
                      </div>
                    )}
                  </div>
                </div>
...
function DayView({ clientId, norms, ratingByMeal, ratingByDay }: {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.trainerClientStatsRange(clientId, date, date),
  });
        ...
        <>
          ...
          {meals.length > 0 && (
            <div style={{ marginTop: 8, background: 'var(--surface)', borderRadius: 'var(--r-md)', padding: '12px 14px', border: '1px solid var(--border)' }}>
              <DayRatingBar date={date} clientId={clientId} existingRating={ratingByDay[date]} />
            </div>
          )}
        </>
      )}
...
function WeekView({ clientId, norms, ratingByDay }: {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['trainer-client-week', clientId, fromStr, toStr],
    queryFn: () => api.trainerClientStatsRange(clientId, fromStr, toStr),
  });
  ...
  const { totals, activeDays, byDate } = useMemo(() => {
    ...
    return { totals: { cal: Math.round(cal), p, f, c }, activeDays: Object.keys(byDate).length, byDate };
  }, [meals]);
    ...
    <>
      ...
      {activeDays === 0 ? (
        <div style={{ textAlign: 'center', padding: '48px 16px' }}>
          ...
          <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-2)', marginBottom: 4 }}>Нет данных за неделю</div>
          <div style={{ fontSize: 12, color: 'var(--text-3)' }}>Клиент не записывал питание в этот период</div>
        </div>
      ) : (
        <>
          {/* Period summary */}
                ...
                <div key={day}>
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      {ratingByDay[day] && (() => {
                        const chip = DAY_RATING_CHIP[ratingByDay[day]];
                        return chip ? (
                          <span style={{
...
function WeightView({ history, profile }: {
  history: Array<{ id: number; weightKg: number; createdAt: string }>;
    ...
    <>
      <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
        <div style={{ flex: 1, background: 'var(--surface)', borderRadius: 'var(--r-lg)', padding: '16px', border: '1px solid var(--border)' }}>
          ...
          <div style={{ fontSize: 28, fontWeight: 700, letterSpacing: -0.8, color: 'var(--text)', lineHeight: 1 }}>
            ...
            {currentWeight != null && <span style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-3)' }}> кг</span>}
          </div>
        </div>
        <div style={{ flex: 1, background: 'var(--surface)', borderRadius: 'var(--r-lg)', padding: '16px', border: '1px solid var(--border)' }}>
      ...
      {history.length === 0 ? (
        <div style={{ background: 'var(--surface)', borderRadius: 'var(--r-lg)', border: '1px solid var(--border)', padding: '32px 16px', textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--text-3)' }}>Клиент ещё не вносил данные о весе</div>
        </div>
      ) : (
          ...
          {history.map((entry, i, arr) => {
            ...
            return (
              ...
              >
                ...
                {delta !== null && (
                  <span style={{ fontSize: 12, fontWeight: 600, color: weightDeltaColor(delta) }}>
...
export default function CoachClientStatsScreen() {
  ...
  const norms = {
    ...
    p:   profile?.dailyProteinG    ?? null,
    f:   profile?.dailyFatG        ?? null,
    c:   profile?.dailyCarbsG      ?? null,
  };
